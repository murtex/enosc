...
#include <enosc/kernels.h>
	...
	/* con/destruction */
enosc::RoesslerMod2::RoesslerMod2()
{
	...
	/* configuration */
void enosc::RoesslerMod2::configure( libconfig::Config const & config, std::string const & groupname )
{
...
void enosc::RoesslerMod2::init()
{
	...
	/* ode */
bool enosc::RoesslerMod2::compute_deriv_det( enosc::device_vector const & state, enosc::scalar time )
{

		/* safeguard */
	if ( state.size() != _state.size() )
		throw std::runtime_error( "invalid argument (enosc::RoesslerMod2::compute_deriv_det): state" );
	...
	thrust::for_each_n(
		...
		_epsilons.size() * _betas.size() * _size, enosc::RoesslerMod2ODE( _a, _b, _c ) );
