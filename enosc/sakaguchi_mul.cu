...
#include <enosc/kernels.h>
	...
	/* con/destruction */
enosc::SakaguchiMul::SakaguchiMul()
{
	...
	/* configuration */
void enosc::SakaguchiMul::configure( libconfig::Config const & config, std::string const & groupname )
{
...
void enosc::SakaguchiMul::init()
{
	...
	/* ode */
bool enosc::SakaguchiMul::compute_deriv_det( enosc::device_vector const & state, enosc::scalar time )
{

		/* safeguard */
	if ( state.size() != _state.size() )
		throw std::runtime_error( "invalid argument (enosc::SakaguchiMul::compute_deriv_det): state" );
	...
	thrust::for_each_n(
		...
		_epsilons.size() * _betas.size() * _size, enosc::SakaguchiMulDetODE( _gamma ) );

	return true;
}

bool enosc::SakaguchiMul::compute_deriv_stoch( enosc::device_vector const & state, enosc::scalar time )
{

		/* safeguard */
	if ( state.size() != _state.size() )
		throw std::runtime_error( "invalid argument (enosc::SakaguchiMul::compute_deriv_stoch): state" );
	...
	thrust::for_each_n(
		...
		_epsilons.size() * _betas.size() * _size, enosc::SakaguchiMulStochODE( _gamma ) );
